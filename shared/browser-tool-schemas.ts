...
const BROWSER_MODIFIER_KEYS = ["Alt", "Control", "ControlOrMeta", "Meta", "Shift"] as const;
const BROWSER_MODIFIER_ALIASES = ["Ctrl", "Cmd", "Command", "Option"] as const;
const BROWSER_MOUSE_BUTTONS = ["left", "middle", "right", "back", "forward"] as const;
const BROWSER_POINTER_MODIFIERS = BROWSER_MODIFIER_KEYS;
const BROWSER_PRESS_KEY_BASES = [...BROWSER_BASE_KEYS, ...BROWSER_KEY_ALIASES] as const;
const BROWSER_PRESS_KEY_MODIFIERS = [...BROWSER_MODIFIER_KEYS, ...BROWSER_MODIFIER_ALIASES] as const;
const BROWSER_SUPPORTED_KEYS = [
  ...BROWSER_PRESS_KEY_BASES,
  ...BROWSER_PRESS_KEY_MODIFIERS,
  ...BROWSER_PRESS_KEY_MODIFIERS.flatMap((modifier) =>
    BROWSER_PRESS_KEY_BASES.map((key) => `${modifier}+${key}`)
  ),
  ...BROWSER_PRESS_KEY_MODIFIERS.flatMap((first, firstIndex) =>
    BROWSER_PRESS_KEY_MODIFIERS.slice(firstIndex + 1).flatMap((second) =>
      BROWSER_PRESS_KEY_BASES.map((key) => `${first}+${second}+${key}`)
    )
  )
] as const;
...
] as const satisfies readonly BrowserToolSchema[];
export const browserActionParameterSchemas = Object.fromEntries(
  browserActionRegistry.map((entry) => {
...
) as Record<BrowserAction, JsonSchema>;

export type BrowserActionParamsValidationResult =
  | { ok: true }
  | { ok: false; code: "invalid_params"; message: string; path: string };

export function getBrowserActionParameterSchema(action: BrowserAction): JsonSchema {
...
export function validateBrowserActionParams(
  action: BrowserAction,
  params: unknown
): BrowserActionParamsValidationResult {
...
function validateFileChooserIdParams(params: unknown): BrowserActionParamsValidationResult {
  if (!params || typeof params !== "object" || Array.isArray(params)) {
    return invalid("$", "must be an object");
  }

  const source = params as Record<string, unknown>;
  const hasFileChooserId =
    (typeof source.fileChooserId === "string" && source.fileChooserId.trim().length > 0) ||
...
function validateUploadFilePathParams(params: unknown): BrowserActionParamsValidationResult {
  if (!params || typeof params !== "object" || Array.isArray(params)) {
    return invalid("$", "must be an object");
  }
  ...
  const hasFilePaths =
    Array.isArray(source.filePaths) &&
...
function invalid(path: string, reason: string): BrowserActionParamsValidationResult {
  return {
    ok: false,
    code: "invalid_params",
    message: `${path} ${reason}`,
