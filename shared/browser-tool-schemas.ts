...
const BROWSER_SUPPORTED_KEYS = [
  ...
  ...BROWSER_MODIFIER_KEYS.flatMap((modifier) =>
    BROWSER_BASE_KEYS.map((key) => `${modifier}+${key}`)
  ),
  ...BROWSER_MODIFIER_KEYS.flatMap((first, firstIndex) =>
    BROWSER_MODIFIER_KEYS.slice(firstIndex + 1).flatMap((second) =>
      BROWSER_BASE_KEYS.map((key) => `${first}+${second}+${key}`)
    )
  )
] as const;
