(() => {
  const pageWindow = window as Window & {
    __agentBrowserControllerContentInstalled?: boolean;
  };
  ...
  function applyPhase(nextPhase: unknown) {
  ...
  function normalizePhase(value: unknown): CursorPhase {
    return value === "active" ||
      value === "thinking" ||
      value === "idle" ||
      value === "handoff" ||
  ...
  function isPersistentPageStatus(value: CursorPhase) {
  ...
  function setFaviconBadge(nextPhase: CursorPhase | null, faviconDataUrl: string | null) {
    ...
    let link = document.getElementById(faviconBadgeId) as HTMLLinkElement | null;
  ...
  function showClickRipple(point: Point) {
  ...
  function showHighlightRect(rect: unknown, options: Record<string, unknown> = {}) {
