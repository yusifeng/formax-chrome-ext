function extParam(name, kind, required = false) {
...
const EXTENSION_ACTION_PARAM_SPECS = {
    health: [
...
const EXTENSION_SUPPORTED_KEYS = [
    ...
    ...EXTENSION_PRESS_KEY_MODIFIERS.flatMap((modifier) => EXTENSION_PRESS_KEY_BASES.map((key) => `${modifier}+${key}`)),
    ...EXTENSION_PRESS_KEY_MODIFIERS.flatMap((first, firstIndex) => EXTENSION_PRESS_KEY_MODIFIERS.slice(firstIndex + 1).flatMap((second) => EXTENSION_PRESS_KEY_BASES.map((key) => `${first}+${second}+${key}`)))
];
const EXTENSION_ACTION_PARAM_ENUMS = {
    updatePolicy: {
...
function validateExtensionActionParams(action, params) {
    ...
    validateExtensionParamEnums(`${action}.params`, params, EXTENSION_ACTION_PARAM_ENUMS[action] || {});
}
function validateExtensionParamSpecs(action, path, params, specs) {
...
function validateExtensionParamKind(action, path, kind, value) {
    if (kind === "string") {
...
function validateExtensionLocator(action, path, value) {
    if (!isExtensionPlainObject(value)) {
        throw new Error(`${path} must be an object`);
    }
...
function validateExtensionStringArray(path, value) {
    if (!Array.isArray(value)) {
        throw new Error(`${path} must be an array`);
    }
    for (let index = 0; index < value.length; index += 1) {
...
function validateExtensionObjectArray(path, value) {
    if (!Array.isArray(value)) {
        throw new Error(`${path} must be an array`);
    }
    for (let index = 0; index < value.length; index += 1) {
...
function validateExtensionParamEnums(path, params, enums) {
...
function isExtensionPlainObject(value) {
