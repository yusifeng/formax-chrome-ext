(() => {
    ...
    function setFaviconBadge(nextPhase, faviconDataUrl) {
        if (!document.head) {
            return;
        }
        if (nextPhase == null) {
    ...
    function normalizeFaviconBadge(value) {
    ...
    function addPendingApproval(value) {
    ...
    function normalizePendingApproval(value) {
        ...
        return {
            ...
            reasons: Array.isArray(source.reasons)
                ? source.reasons.filter((reason) => typeof reason === "string")
                : undefined,
            ...
            subject: source.subject &&
                ...
                ? source.subject
                : undefined,
            suggestedDecisions: source.suggestedDecisions &&
    ...
    function createApprovalDetails(approval) {
    ...
    function createApprovalButtons(approval) {
        ...
        if (allowForSession) {
            buttons.push(createApprovalButton(approval, "approve", "Allow session", hostPolicyDecision(allowForSession, "allow")));
        }
        buttons.push(createApprovalButton(approval, "approve", "Always allow", hostPolicyDecision(approval.suggestedDecisions?.alwaysAllow, "always_allow")));
        return buttons;
    }
    function createApprovalButton(approval, decision, labelText, policyDecision) {
    ...
    async function resolveApprovalFromContent(approval, decision, policyDecision) {
        try {
    ...
    function approvalTitle(approval) {
        if (approval.kind === "host")
            return "Website access request";
        if (approval.kind === "origin")
            return "Origin approval request";
    ...
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
        ...
        if (message.type === "TAB_FAVICON_BADGE") {
            ...
            const faviconDataUrl = typeof message.faviconDataUrl === "string" ? message.faviconDataUrl : null;
