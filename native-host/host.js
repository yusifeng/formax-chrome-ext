...
function handleChromeMessage(message) {
    ...
    const typed = message;
    if (typed.type === "hello") {
        console.error(`[native-host] connected to extension ${typed.extensionId}, version ${typed.version}`);
        return;
    }
    if (typed.type === "event") {
    ...
    const messageText = typed.error?.message || JSON.stringify(typed.error) || "Unknown extension error";
...
server.listen(HTTP_PORT, HTTP_HOST, () => {
    console.error(`[native-host] HTTP RPC listening on http://${HTTP_HOST}:${HTTP_PORT}/rpc`);
});
...
async function validateActionParams(action, params) {
