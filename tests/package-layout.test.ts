...
describe("package layout", () => {
  ...
  it("exposes the stable packaged browser client entrypoint", async () => {
    const moduleUrl = pathToFileURL(
      path.join(root, "build", "runtime", "scripts", "browser-client.mjs")
    ).href;
    const browserClient = await import(moduleUrl);
    const source = read("build/runtime/scripts/browser-client.mjs");
  ...
  it("bundles the MCP runtime entrypoints into self-contained files", () => {
    const serverSource = read("build/runtime/mcp-node-repl/server.js");
    const kernelChildSource = read("build/runtime/mcp-node-repl/kernel-child.js");
  ...
  it("keeps shipped debug scripts compatible with the packaged layout", () => {
    const distSmoke = read("dist/tests/scripts/mcp-node-repl-smoke.js");

    expect(distSmoke).toContain('"mcp-node-repl/server.js"');
    expect(distSmoke).toContain('"./scripts/browser-client.mjs"');
    expect(fs.existsSync(path.join(root, "dist", "tests", "scripts", "llm-node-repl-chat.js"))).toBe(false);
  });
});
