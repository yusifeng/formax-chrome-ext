...
describe("browser tool schemas", () => {
  ...
  it("keeps protocol docs action sections aligned with the shared action registry", () => {
    ...
    expect(protocol).toContain("Keep `shared/action-registry.ts`, `shared/browser-tool-schemas.ts`");
  });
  ...
  it("documents the node_repl browser-use operating model in skill, API docs, and packaged runtime docs", () => {
    const client = readFileSync("mcp-node-repl/browser-client.ts", "utf8");
    const api = readFileSync("docs/api.md", "utf8");
    const skill = readFileSync("skill/SKILL.md", "utf8");
    const todo = readFileSync("notes/codex-gap-todolist.md", "utf8");

    expect(client).toContain('return readFile(new URL(`${name}.md`, rootUrl), "utf8")');
    expect(client).toContain('const documentationTopics = [');
    expect(api).toContain('await agent.documentation.get("playwright")');
    ...
    expect(todo).toContain("runtime documentation API");
  });
